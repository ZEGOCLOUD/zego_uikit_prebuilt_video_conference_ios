...
protocol ZegoVideoConferenceDarkBottomMenuBarDelegate: AnyObject {
    func onMenuBarMoreButtonClick(_ buttonList: [UIView])
    func onLeaveVideoConference(_ isLeave: Bool)
}

extension ZegoVideoConferenceDarkBottomMenuBarDelegate {
    func onMenuBarMoreButtonClick(_ buttonList: [UIView]) { }
    func onLeaveVideoConference(_ isLeave: Bool){ }
}

class ZegoVideoConferenceDarkBottomMenuBar: UIView {
    ...
    public weak var delegate: ZegoVideoConferenceDarkBottomMenuBarDelegate?
    ...
    public func addButtonToMenuBar(_ button: UIButton) {
        if self.buttons.count > self.config.bottomMenuBarConfig.maxCount - 1 {
            if self.buttons.last is VideoConferenceMoreButton {
                self.moreButtonList.append(button)
                return
            }
            //替换最后一个元素
            let moreButton: VideoConferenceMoreButton = VideoConferenceMoreButton()
...
extension ZegoVideoConferenceDarkBottomMenuBar: LeaveButtonDelegate {
    func onLeaveButtonClick(_ isLeave: Bool) {
        delegate?.onLeaveVideoConference(isLeave)
        if isLeave {
...
class VideoConferenceMoreButton: UIButton {
    ...
    override init(frame: CGRect) {
        super.init(frame: frame)
        self.setImage(ZegoUIKitVideoConferenceIconSetType.icon_more.load(), for: .normal)
    }
